...
#if __CH_VERSION_OP_H__ != 202602L
#error Bad 'op' version.
#else
...
static void op__lines(const char *lines[], int width) {
  if (lines == nullptr) {
    return;
  }
  const char **pp = lines;
  while (*pp != nullptr) {
    op__help_line(*pp, 0, 0, width);
    ++pp;
  }
}

static void op__help(bool isprepromp,
    const opitem *base, size_t nmemb,
    int tab1, int tab2, int sptab2bfr, int sptab2aftr,
    const char *preprompt[], const char *posthelp[]) {
  struct winsize ws;
  if (ioctl(0, TIOCGWINSZ, &ws) == -1) {
    ws.ws_col = WIDTH_DEFAULT;
  }
  if (isprepromp) {
    op__lines(preprompt, ws.ws_col);
  } else {
...
int op_execute(opreturn (*open)(void *), void (*close)(void *),
  ...
  op__help(true,
      base, nmemb,
      tab1, tab2, sptab2bfr, sptab2aftr,
      preprompt, posthelp);
  int c = '\n';
loop:
  if (c == '\n') {
